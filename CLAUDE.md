...
- Original skills must be self-contained: never link to files in sibling skill directories — the skills CLI installs skills individually with no dependency resolution, so cross-skill links dangle. Vendor a copy into the skill's own directory with a provenance comment instead. (In replicated skills, keep the prose verbatim but repoint cross-skill links to vendored copies, and note the deviation in the README.)
