...
bash scripts/check_replicated_skills.sh [skill...]   # diff replicated skills against upstream; exit 1 on drift
```
...
- Replicated skills are tracked in `scripts/replicated-skills.json` (upstream repo, ref, upstream→local path map, and an `ignore` list for files vendored from sibling skills). `scripts/check_replicated_skills.sh` diffs each against upstream; `.github/workflows/replicated-skills-drift.yml` runs it on PRs (posting a sticky comment with the diff), weekly, and on dispatch, and fails on any drift. When adding a replicated skill: copy it byte-for-byte, add it to the manifest, **and** add its directory to the `replicated_skills` exclude regex in `.pre-commit-config.yaml` — the markdownlint/whitespace fixers otherwise rewrite the copy and the drift check goes red. Resync by re-copying from upstream, never by hand-editing.
